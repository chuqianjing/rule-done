...
各地、各基层使用的模板不尽相同，因此工具并未提供可直接使用的模板。各支部需要结合自身实际，按如下步骤制定模板、并将其存放至指定目录。

>注：地方特定等级党委下属的各支部通常会使用相同模板，建议同属各支部针对该等级统一制定模板，减少重复工作。

#### 标记占位符

汇总发展流程全部材料为 `.docx` 格式的文档，使用形如 `{{代填项名称}}` 的占位符在代填项处进行标记。请务必注意以下标记规则：
  - 工具的「模板详情」页展示的项名即为 `{{...}}` 中的“代填项名称”；
  - 必要时，请使用短下划线“_”来分隔“代填项名称”的语义，例如“党支部意见_内容”；
  - “代填项名称”中包含特定字符时，界面对该项的呈现样式会有不同：
    ```bash
     "要求"、"填写要求"：15行左右的文本类型
     "内容"、"填写内容"：3行左右的文本类型
     "简历"、"优缺点"：5行左右的文本类型
...
>注：有些统一印制、需要手写的文件并不需要本工具来导出为填好的材料，但也建议管理员新建其对应的 `.docx` 文档，并在其中标记所有的代填项、以便成员直接参照来手写材料，或者也可以仅填入类似“要求”的占位符。

#### 编写配置文件

访问相应网站下载 [templates_config.json](https://github.com/chuqianjing/rule-done-content/blob/main/templates_config.json)，并在该示例基础上修改。该文件用以在工具界面中标记所有模板文件的阶段、用途等信息。建议支部根据本地工作实际，认真编写该文件，增强成员对党员发展工作的严肃认识和组织意识。该文件中各字段含义说明如下：
```json
{
  "stages": [
    {
...
在「通用设置」页的「管理材料模板」分组，将 `templates_config.json` 和所有 `.docx` 文档放置到打开的文件夹中。重启工具，即可使用模板。

<div align="center">
  <img src="photos/模板放入指定目录.png" width="100%" alt="alt text">
</div>
...
由于资金有限，工具目前是借助第三方平台来实现双端通信的。一方面，管理员将配置文件 `admin_config.json` 推送至远程仓库、成员端自动拉取该文件到本地；另一方面，成员基本信息可以自动传输到管理员管理的在线表格。这两方面均须管理员设置相关同步项，这些同步项也会保存在配置文件中以供成员端使用。
...
- 特殊字段：需在表中添加“预期进度”字段，该信息会呈现在成员端「模板列表」页
...
本工具提供了GitHub和阿里云OSS两种云存储方式，用户可按需选择。在「通用设置」页的「本地配置文件同步至远程」分组，选择同步目标并配置相关参数，点击「立即同步到远程」即可保存相关参数并发布配置到远程存储仓库。由于远程仓库只有设为公开成员端方能访问，为确保配置文件不被恶意使用，强烈建议在推送至远程前为其设置加密保护。
...
首次使用时，需要获取所在支部的管理员配置文件 `admin_config.json`，且默认需要为该文件的远程获取设置解密密钥。
...
分析任务场景，确定个体基本项、整体基本项、模板常见项，并制定每项的规则，编写字段规则文件 `fields_definition.json`，将其放入 `resources/schema/` 目录下（该目录与材料模板文件夹的是同级的）。以就业类的表格材料为例，可能的字段规则定义如下：
...
### 2、准备模板
按照[党支部管理员操作指南中的相关步骤](#1制定模板)制定模板。
