...
**「入档 • 党员发展档案管理工具」** 是一款基于 `PySide6` 开发的桌面应用程序，旨在解决党员发展过程中 **沟通协调效率较低、材料信息重复录入、关键节点遗忘易错、档案备份不慎丢失** 的痛点。它将党员发展全流程的材料与信息集中管理，通过党支部管理员和发展成员协作，基于**字段场景化映射**和**文档占位符替换**技术，让信息「录入一次」即可复用于成员的「多个」材料——即便材料跨越数年陆续产生，前后信息与节点进度也能保持一致。在档案管理工作的**管信息**和**填材料**两大方面具备如下优势：
...
以下呈现了工具如何区分并使用各类数据。其中，字段定义和模板与模板配置即资源，管理员配置和成员信息是业务场景的真实具体数据。

| 数据 | 是什么 | 谁维护 | 何时有效 | 在哪里 |
| --- | --- | --- | --- | --- |
| 字段定义 `fields_definition.json` | 对实际信息的格式要求 | 管理员 | 长期 | 本地 + 远程同步 |
| 模板与模板配置 `templates/` | 材料的形式载体 | 管理员 | 长期 | 本地 + 远程同步 |
| 管理员配置 `admin_config.json` | 支部级共性数据（包括基本配置和模板配置） | 管理员 | 按批次和快照留存 | 本地 + 远程同步 |
| 成员信息 `member_info.json` | 成员级个性数据（包括基本信息和模板信息） | 成员 | 全程随写随存 | 本地 + 基本信息汇总同步 |
| 系统设置 `system_settings.json` | 运行时数据 | 系统 | 实时 | 本地 |
