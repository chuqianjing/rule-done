...
1. **信息汇总（成员 → 管理员）**：成员基本信息自动传输至由管理员管理的在线表格；同时管理员在表格中维护的“进度提醒”等字段会回填至成员端；
...
> **注意：三种平台三选一即可。** 工具**只保存当前「成员信息汇总平台」所选平台的凭据**：切换平台并保存后，原平台的凭据会被从本地配置中移除（也不会随配置发布到远程），需要切回时须重新填写。
